export const StartButton = ({ children = 'Go to Quickstart' }) => {
  return (
    <a className="group" href="/quickstart">
      <button className="flex items-center space-x-1 font-medium px-4 py-1 bg-primary-dark dark:bg-primary-light rounded-xl group-hover:opacity-[0.9] text-white dark:text-black group-hover:opacity-[0.9]">
        <span>{children}</span>
        <svg
...
export const CTAButton = ({ children = 'Get Your API Key' }) => {
  return (
    <a className="group" href="https://www.llmradar.ai/api">
      <button className="flex items-center space-x-1 font-medium px-4 py-1 bg-primary-dark dark:bg-primary-light rounded-xl group-hover:opacity-[0.9] text-white dark:text-black group-hover:opacity-[0.9]">
        <span>{children}</span>
        <svg
          xmlns="http://www.w3.org/2000/svg"
